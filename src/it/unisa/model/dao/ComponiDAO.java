...
public class ComponiDAO {
	...
	public void doSave(OrdineBean ordine,Carrello carrello) throws SQLException {
		...
		String insertComporreSQL = "insert into "+TABLE_NAME_Comporre+" (ID_Articolo,ID_Ordine,IVA,Prezzo_Articolo,Quantità_Selezionata)"
								  +" values(?,?,?,?,?)";
		try {
	...
	public ArrayList<ComponiBean> doRetrieveByKey(int idOrdine) throws SQLException{
		...
		try {
			...
			while(result.next()) {
				...
				bean.setQuantita(result.getInt("Quantità_Selezionata"));
